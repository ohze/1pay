...
class SmsCheckSpec extends PlaySpecification {
  "/1pay/check" should {
    ...
    "ok" in new WithApplication {
      ...
      (js \ "type").asOpt[String] must beSome("text")
      (js \ "sms").asOpt[String] must beSome.which(
        _.startsWith("Tin nhan dung cu phap.")
      )
    ...
    "ok with real data" in new WithApplication {
      ...
      val signature =
        "3c5aa7b2f8254db41fe1cc7d4980003f93316e8adc4271a113bb132c556491ed"
      SignData.sign(
      ...
      implicit val req = FakeRequest(
        GET,
        s"/1pay/check?access_key=4l732pw3tjhzexml8wry&amount=5000&command_code=SD&mo_message=SD+NAP+14&msisdn=84982479988&signature=$signature&telco=vtm"
      )
